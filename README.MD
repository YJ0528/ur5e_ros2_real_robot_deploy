...
2.  Call the service to move the robot to the respective pose (choose one below):
	...
	Watch video: [moveit_target_pose_control.mp4](https://drive.google.com/file/d/1yuX_eNz4BOV9o3eFzmlRvPEuQVXk8LWY/view?usp=sharing)
...
3.  **(Optional)** To change the tracking pose, change the parameter during runtime to set the node to publish differnet pose message to `/target_pose` topic (choose one below)"
    ...
    Watch video: [target_pose_tracking_demo.mp4](https://drive.google.com/file/d/1rRxkjDuF2KTgoPr9KcjpE_QdbPpTxLNG/view?usp=sharing)
...
| `/stop` | `bool` | `moveit_target_pose_control` \<sub> | Signal to stop the robot and exit the service |
